...
asset_root = Path('/mnt/hdd/data/SynBody/SMPL-XL-100-fbx/0000001').resolve()
meta_root = Path('/mnt/hdd/data/SynBody/SMPL-XL-100-meta/0000001').resolve()
amass_root = Path('/mnt/hdd/data/AMASS/SMPL-X_N').resolve()
...
# amass_file = amass_root / 'BMLrub/rub037/0027_circle_walk_stageii.npz'
amass_file = Path('/mnt/hdd/data/AMASS/SMPL-X_N/ACCAD/Male2Running_c3d/C5_-_walk_to_run_stageii.npz').resolve()
...
smpl_xl_file = asset_root / 'SMPL-XL-baked.fbx'
...
smpl_xl_meta_file = meta_root / 'meta_smplx.npz'
...
output_path = Path('./output/samples/blender').resolve()
...
def main(background: bool = False):
    ...
    xf_runner = xf.init_blender(
        exec_path=engine_exec_path, background=background
    )
