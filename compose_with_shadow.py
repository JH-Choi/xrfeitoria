...
def main(args):
    ...
    fg_img_path = "./0000static_camera_0.png"
    obj_mask_path = "./0000static_camera_0.exr"
    bg_img_path = "./00000.png"

    fg_img = cv2.imread(str(fg_img_path), cv2.IMREAD_UNCHANGED)
    # obj_mask = read_exr(str(obj_mask_path)) 
    obj_mask = cv2.imread(str(obj_mask_path), cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    # pdb.set_trace()
    # cv2.imwrite('obj_mask.jpg', obj_mask * 255)
    obj_mask = obj_mask * 255 > 1
    bg_img = cv2.imread(str(bg_img_path), cv2.IMREAD_UNCHANGED)

    bg_img[obj_mask] = 0
    # bg_img = bg_img * (obj_mask)

    shadow_catcher_alpha = fg_img[:,:,3:] / 255.
    # cv2.imwrite('bg_tmp.jpg', bg_img)

    composite_image = bg_img * (1-shadow_catcher_alpha) + fg_img[:,:,0:3] * (shadow_catcher_alpha)
    # composite_image = bg_img * (shadow_catcher_alpha) + fg_img[:,:,0:3] * (1-shadow_catcher_alpha)
    cv2.imwrite('tmp.jpg', composite_image)

    shadow_catcher_alpha[obj_mask[..., :1]] = 0
    cv2.imwrite('shadow_catcher.jpg', (1 - shadow_catcher_alpha)* 255)
